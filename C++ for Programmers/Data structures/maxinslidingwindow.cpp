...
vector<int> FindMaxSlidingWindow(vector<int>& nums, int window_size) {
	...
	//set window size to nums.size()
	if (window_size > nums.size()) {
		window_size = nums.size();

	}

	//initializing double ended queue for storing indices
	deque<int> window;

	//finding out first maximum in the window size
	for (int i = 0; i < window_size; ++i) {
		//for every element, remove the previous smaller element
	...
	//apppending the largest element in the window to the result
